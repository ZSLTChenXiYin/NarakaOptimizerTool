...
func postPhysic(c *gin.Context) {
	physic := c.DefaultPostForm("physic", "off")
	if developerMode() {
		DebugLogger.Printf(NewLog("Physic: %s", physic))
	}

	quality_settings_data_file_path := "/program/NarakaBladepoint_Data/QualitySettingsData.txt"

	file, err := os.OpenFile(narakaInstallPath()+quality_settings_data_file_path, os.O_RDWR, 0666)
	if err != nil {
		if developerMode() {
...
func postInitialization(c *gin.Context) {
	...
	developer_mode := c.DefaultPostForm("developer_mode", "off")
	if developerMode() {
